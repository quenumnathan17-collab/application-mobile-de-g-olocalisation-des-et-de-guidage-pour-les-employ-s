...
export default function AdminDashboard({
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setGeocodingAlert({
          type: "success",
          text:
            `Position GPS detectee : Lat ${lat.toFixed(6)}, ` +
            `Lng ${lng.toFixed(6)} (precision: ` +
      ...
      (error) => {
        ...
        if (error.code === 1)
          msg =
            "Permission de géolocalisation refusée. Autorisez l'accès dans les paramètres de votre navigateur.";
        else if (error.code === 2)
          msg =
            "Position indisponible. Vérifiez que le GPS est activé sur votre appareil.";
        else if (error.code === 3)
          msg = "Délai de géolocalisation dépassé. Réessayez.";
          ...
          setTimeout(() => {
            if (mapInstance.current) {
              ...
              const tempMarker = L.marker([lat, lon])
                ...
                .bindPopup(
                  `<b>Résultat de recherche :</b><br>${data[0].display_name}`,
                )
                .openPopup();
  ...
  const handleAddressAutocomplete = (value) => {
    ...
    autocompleteTimer.current = setTimeout(async () => {
      try {
        const commune = newClientCommune
          ? `, ${newClientCommune}, Abidjan`
          : ", Abidjan";
        const url =
          `https://nominatim.openstreetmap.org/search?q=` +
          `${encodeURIComponent(value + commune)}` +
          `&countrycodes=ci&format=json&limit=5`;
        const res = await fetch(url, {
          headers: { "Accept-Language": "fr" },
        });
        const data = await res.json();
    ...
    try {
      ...
      if (showManualGps && manualLat && manualLng) {
        ...
        if (
          ...
          setGeocodingAlert({
            type: "error",
            text: "Coordonnées GPS invalides. Vérifiez la latitude (ex: 5.3211) et la longitude (ex: -4.0180).",
          });
        ...
        if (!location) {
          setGeocodingAlert({
            type: "error",
            text:
              "Adresse introuvable après plusieurs tentatives. " +
              "Astuce : essayez un nom de rue connu, ou activez " +
              "la saisie GPS manuelle pour entrer les coordonnées " +
              "directement.",
          });
          setGeocodingLoading(false);
          return;
      ...
      setGeocodingAlert({
        ...
        text:
          ` Géocodage réussi${strategyMsg} — Lat ` +
          `${savedClient.gps.lat.toFixed(4)}, Lng ` +
          `${savedClient.gps.lng.toFixed(4)}`,
      });
    ...
    try {
      ...
      if (editClientData.manualLat && editClientData.manualLng) {
        const lat = parseFloat(
          String(editClientData.manualLat).replace(",", "."),
        );
        const lng = parseFloat(
          String(editClientData.manualLng).replace(",", "."),
        );
  ...
  // Update Map Markers
  useEffect(() => {
    if (activeTab !== 0 || !mapInstance.current || !markersGroup.current)
      return;
    ...
    clients.forEach((client) => {
      ...
      if (legendFilter === "particulier" && client.type !== "particulier")
        return;

      const clientOps = operations.filter(
        (op) =>
    ...
    employees.forEach((emp) => {
      ...
      if (legendFilter === "entreprise" || legendFilter === "particulier")
        return; // hide techs when filtering clients only

      const activeOps = operations.filter(
        (op) => op.employeeId === emp.id && op.status === "en cours",
      );
      ...
      >
        ...
        >
          ...
          >
            <Tab
              icon={
                <MapIcon sx={{ fontSize: 20, transition: "transform 0.25s" }} />
              }
              iconPosition="start"
              label="Vue Terrain"
            ...
            <Tab
              icon={
                <PeopleIcon
                  sx={{ fontSize: 20, transition: "transform 0.25s" }}
                />
              }
              iconPosition="start"
        ...
        <Box sx={{ flex: 1, overflowY: "auto", p: 3 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              ...
              >
                <Box>
                  ...
                  >
                    Suivez l'activité de vos équipes en temps réel et gardez un
                    œil sur les interventions de la journée. Cliquez sur un
                    filtre pour isoler un type de marqueur.
                  </Typography>
                </Box>
                <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                  <Chip
                    ...
                    size="small"
                    variant={
                      legendFilter === "entreprise" ? "filled" : "outlined"
                    }
                    color={
                      legendFilter === "entreprise" ? "primary" : "default"
                    }
                    onClick={() =>
                      setLegendFilter(
                        legendFilter === "entreprise" ? "all" : "entreprise",
                      )
                    }
                  ...
                  <Chip
                    ...
                    size="small"
                    variant={
                      legendFilter === "particulier" ? "filled" : "outlined"
                    }
                    color={
                      legendFilter === "particulier" ? "secondary" : "default"
                    }
                    onClick={() =>
                      setLegendFilter(
                        legendFilter === "particulier" ? "all" : "particulier",
                      )
                    }
                    sx={{ fontWeight: 600, cursor: "pointer" }}
                  />
                  <Chip
                    ...
                    size="small"
                    variant={
                      legendFilter === "technicien" ? "filled" : "outlined"
                    }
                    color={
                      legendFilter === "technicien" ? "success" : "default"
                    }
                    onClick={() =>
                      setLegendFilter(
                        legendFilter === "technicien" ? "all" : "technicien",
                      )
                    }
                    sx={{ fontWeight: 600, cursor: "pointer" }}
                  />
          ...
          {activeTab === 1 && (
            <Box
              sx={{ display: "flex", flexDirection: "column", gap: 4, pt: 1 }}
            >
              <Box
                sx={{
                  display: "flex",
              ...
              >
                <Box>
                  ...
                  >
                    Gérez votre portefeuille client. L'adresse est géocodée et
              ...
              <Paper
                sx={{
                  ...
                  background: (theme) =>
                    theme.palette.mode === "dark"
                      ? "linear-gradient(180deg, #151c2c 0%, #0b0f19 100%)"
                        ...
                        >
                          ...
                          <TableCell sx={{ whiteSpace: "nowrap" }}>
                            <Chip
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setClientTypeFilter(client.type);
                              }}
                            />
                          </TableCell>
                          <TableCell sx={{ whiteSpace: "nowrap" }}>
                          ...
                          <TableCell
                            onClick={(e) => {
                              e.stopPropagation();
                              centerMapOnGps(client.gps.lat, client.gps.lng);
                            }}
                            sx={{
                              cursor: "pointer",
                              whiteSpace: "nowrap",
                          ...
                          >
                            ...
                            >
                              ...
                              {client.gps.lat.toFixed(4)},{" "}
                              {client.gps.lng.toFixed(4)}
                            </Box>
                          </TableCell>
                          <TableCell
                            align="right"
                          ...
                          <TableCell colSpan={5} align="center" sx={{ py: 8 }}>
                            ...
                            >
                              ...
                              <Typography variant="body2">
                                Essayez de modifier votre recherche ou vos
                                filtres.
                              </Typography>
                            </Box>
                          </TableCell>
              ...
              >
                <Box>
                  ...
                  <Typography
                    ...
                    sx={{ color: "text.secondary", mt: 0.5 }}
                  >
                    Assignation des missions et suivi du statut des
                    interventions.
                  </Typography>
                </Box>
                <Button
                          ...
                          >
                            <TableCell sx={{ fontWeight: 600 }}>
                              <Typography
                                ...
                                onClick={() =>
                                  client && navigateToClient(client.id)
                                }
                              >
                            ...
                            <TableCell>
                              <FormControl size="small" sx={{ width: 160 }}>
                                <Select
                                  value={op.status}
                                  onChange={(e) =>
                                    updateOperationStatus(op.id, e.target.value)
                                  }
                                  MenuProps={{
                                    PaperProps: {
                                      sx: {
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    doc.text(
                      "Téléphone : (225) 01 52 22 63 12 / 05 65 24 69 74",
                      46,
                      36,
                    );
                    ...
                    doc.text("RAPPORT D'INTERVENTIONS", pageWidth - 14, 18, {
                      align: "right",
                    });
                    ...
                    const docId = `REF-${Math.floor(Math.random() * 10000)
                      .toString()
                      .padStart(4, "0")}-${new Date().getFullYear()}`;
                    doc.text(`Réf : ${docId}`, pageWidth - 14, 25, {
                      align: "right",
                    });
                    doc.text(
                    ...
                    const rate =
                      totalOps > 0 ? Math.round((doneOps / totalOps) * 100) : 0;
                    ...
                    doc.text(
                      "Document généré automatiquement et certifié conforme par le système YA Consulting.",
                      pageWidth / 2,
                      finalY + 12,
                      { align: "center" },
                    );
                    ...
                    doc.save(
                      `Facture_Rapport_YA_Consulting_${new Date().toISOString().slice(0, 10)}.pdf`,
                    );
                  }}
                  ...
                  <TableBody>
                    {employees.map((emp) => (
                      <TableRow
                        ...
                        sx={{
                          "&:last-child td, &:last-child th": { border: 0 },
                          opacity: emp.status === "inactif" ? 0.6 : 1,
                        }}
                      >
                        <TableCell
                        ...
                        <TableCell>
                          <Chip
                            ...
                            size="small"
                            color={
                              emp.role === "admin" ? "secondary" : "default"
                            }
                            variant={
                              emp.role === "admin" ? "filled" : "outlined"
                            }
                            sx={{ fontWeight: 600 }}
                          />
                        </TableCell>
                        ...
                        >
                          ...
                          <IconButton
                            ...
                            onClick={async () => {
                              if (
                                window.confirm(
                                  "Voulez-vous vraiment supprimer définitivement cet employé ?",
                                )
                              ) {
            ...
            <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
              <Box>
                ...
                <Typography
                  ...
                  sx={{ color: "text.secondary", mt: 0.5 }}
                >
                  Retours d'informations des techniciens concernant les
                  imprécisions cartographiques ou les erreurs de coordonnées.
                </Typography>
              </Box>
                    ...
                    {addressReports.map((rep) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {new Date(rep.createdAt).toLocaleString("fr-CI", {
                              day: "numeric",
                              month: "short",
                              hour: "2-digit",
                              minute: "2-digit",
                      ...
                      <TableRow>
                        <TableCell colSpan={4} align="center" sx={{ py: 6 }}>
                          ...
                          >
                            ...
                            <Typography variant="body2">
                              Toutes les adresses sont géocodées correctement !
                            </Typography>
                          </Box>
                        </TableCell>
                      </TableRow>
          ...
          {activeTab === 6 && (
            ...
            >
              ...
              >
                ...
                >
                  Gérez l'identité visuelle de votre entreprise et les
                  paramètres d'accès de vos techniciens.
                </Typography>
                ...
                <Box sx={{ mb: 4 }}>
                  ...
                  >
                    Ce logo apparaîtra dans l'en-tête de votre portail
                    d'interventions.
                  </Typography>
                  ...
                  >
                    ...
                    <Box>
                      <Button
                        variant="outlined"
                        component="label"
                        sx={{ borderRadius: 0, textTransform: "none", mb: 1 }}
                      >
                          ...
                          onChange={async (e) => {
                            ...
                            reader.onloadend = async () => {
                              ...
                              try {
                                const token = localStorage.getItem("token");
                                const res = await fetch(
                                  `${API_URL}/api/organizations/logo`,
                                  {
                                    method: "PUT",
                                    headers: {
                      ...
                      >
                        Fichiers acceptés : PNG, JPG, WebP. Taille maximale : 2
                        Mo.
                      </Typography>
                    </Box>
                  </Box>
                ...
                <Box>
                  ...
                  <Typography
                    ...
                    sx={{ color: "text.secondary", mb: 2 }}
                  >
                    Partagez ce code d'invitation avec vos techniciens pour
                    qu'ils puissent s'inscrire eux-mêmes et rejoindre votre
                    entreprise.
                  </Typography>
          ...
          <form onSubmit={handleAddClientSubmit}>
            ...
            >
              ...
              <Box sx={{ position: "relative" }}>
                <TextField
                  ...
                  helperText={
                    showManualGps
                      ? " Mode GPS manuel activé — coordonnées utilisées à la place."
                      : " Tapez pour voir des suggestions d'adresses à Abidjan."
                  }
                />
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleDeviceGeolocation(setManualLat, setManualLng)
                    }
              ...
              {geocodingAlert && (
                <Alert
                  ...
                  icon={
                    geocodingAlert.type === "success" ? (
                      <CheckCircleIcon />
                    ) : geocodingAlert.type === "warning" ? (
                      <WarningIcon />
                    ) : (
                      <ErrorIcon />
                    )
            ...
            <DialogActions sx={{ p: 2.5, pt: 1.5 }}>
              ...
              <Button
                type="submit"
                variant="contained"
                disabled={saveCompleted || geocodingLoading}
              >
          ...
          {editClientData && (
            <form onSubmit={handleEditClientSubmit}>
              ...
              >
                ...
                <FormControl fullWidth>
                  <InputLabel id="edit-client-type-label">
                    Type de Client
                  </InputLabel>
                  <Select
                ...
                <Autocomplete
                  ...
                  onInputChange={(_, value) =>
                    setEditClientData({ ...editClientData, commune: value })
                  }
                  renderInput={(params) => (
                    <TextField
                      {...params}
                ...
                >
                  <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleDeviceGeolocation(
                          (v) =>
                            setEditClientData((d) => ({ ...d, manualLat: v })),
                          (v) =>
                            setEditClientData((d) => ({ ...d, manualLng: v })),
                          () =>
                            setEditClientData((d) => ({
                              ...d,
                              useManualGps: true,
                    ...
                    <Button
                      size="small"
                      variant={
                        editClientData?.useManualGps ? "contained" : "outlined"
                      }
                      color={
                        editClientData?.useManualGps ? "secondary" : "inherit"
                      }
                      startIcon={<GpsFixedIcon />}
                      onClick={() =>
                        setEditClientData({
                          ...editClientData,
                          useManualGps: !editClientData.useManualGps,
                        })
                      }
                      sx={{ fontSize: "0.8rem" }}
                    >
        ...
        >
          <DialogTitle sx={{ fontWeight: 700, pb: 1 }}>
            Créer et assigner une opération
          </DialogTitle>
          <form onSubmit={handleAddOpSubmit}>
            ...
            >
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl fullWidth required>
                    <InputLabel id="operation-tech-label">
                      Technicien assigné
                    </InputLabel>
                    <Select
                ...
                <>
                    ...
                    <List dense disablePadding>
                      ...
                      >
                        ...
                        <ListItemText
                          ...
                          secondary={
                            `GPS: ${client.gps.lat.toFixed(5)}, ` +
                            `${client.gps.lng.toFixed(5)} — ` +
                  ...
                  <DialogActions sx={{ p: 2.5, pt: 1 }}>
                    <Button
                      ...
                      color={client.archived ? "primary" : "inherit"}
                      onClick={() => {
                        updateClient({ ...client, archived: !client.archived });
                        setSelectedClientDetail(null);
                      }}
                      startIcon={
                        client.archived ? <UnarchiveIcon /> : <ArchiveIcon />
                      }
                      sx={{ mr: "auto" }}
                    >
                      {client.archived ? "Restaurer" : "Archiver"}
        ...
        >
          {selectedEmpDetail &&
            (() => {
              ...
              const empOps = operations.filter(
                (op) => op.employeeId === emp.id,
              );
              const activeOps = empOps.filter((op) => op.status === "en cours");
              const completedOps = empOps.filter(
                (op) => op.status === "terminée",
              );
              return (
                <>
                  <DialogTitle
                  ...
                  <DialogContent sx={{ pt: 2 }}>
                    <List dense disablePadding>
                      ...
                      <ListItem
                        ...
                        onClick={() => {
                          centerMapOnGps(emp.gps.lat, emp.gps.lng);
                      ...
                      >
                        ...
                        <ListItemText
                          primary={`${emp.gps.lat.toFixed(5)}, ${emp.gps.lng.toFixed(5)}`}
                          secondary="Voir la position actuelle sur la carte"
                        />
                      </ListItem>
                    </List>
        ...
        >
          ...
          <form onSubmit={handleAddEmpSubmit}>
            ...
            <DialogActions sx={{ p: 3, pt: 1 }}>
              <Button onClick={() => setShowAddEmp(false)} color="inherit">
                Annuler
              </Button>
