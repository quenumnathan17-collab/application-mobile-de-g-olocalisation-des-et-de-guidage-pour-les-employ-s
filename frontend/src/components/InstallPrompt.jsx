...
export default function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    if (ios) {
      ...
      if (!dismissed) {
        setTimeout(() => setShowBanner(true), 3000); // délai pour ne pas bloquer au démarrage
      }
      return;
    }
    ...
    window.addEventListener('appinstalled', () => { setShowBanner(false); setIsInstalled(true); });
  ...
  const handleDismiss = () => {
    setShowBanner(false);
    localStorage.setItem(isIOS ? 'pwa_ios_dismissed' : 'pwa_install_dismissed', '1');
  };
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 20px 60px rgba(0,0,0,0.4), 0 0 0 1px rgba(105,180,245,0.15) inset',
      display: 'flex',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {isIOS ? (
          <div style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.75)', lineHeight: 1.5 }}>
            Appuyez sur <strong style={{ color: '#69b4f5' }}>
              <Share size={11} style={{ display: 'inline', verticalAlign: 'middle' }} /> Partager
            </strong> puis <strong style={{ color: '#69b4f5' }}>« Ajouter à l'écran d'accueil »</strong>
          </div>
        ) : (
          <>
            <div style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.75)', marginBottom: '0.625rem' }}>
              Installez l'app sur votre appareil pour un accès rapide, même hors ligne.
            </div>
            <button
              ...
              onMouseEnter={e => { e.target.style.transform = 'translateY(-1px)'; e.target.style.boxShadow = '0 6px 16px rgba(59,94,219,0.5)'; }}
              onMouseLeave={e => { e.target.style.transform = ''; e.target.style.boxShadow = '0 4px 12px rgba(59,94,219,0.4)'; }}
            >
